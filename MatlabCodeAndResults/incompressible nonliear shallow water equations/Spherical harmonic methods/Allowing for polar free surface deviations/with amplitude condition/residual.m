    ...
    for m=1:M
	    for l=kappa*m:2*M+kappa*m-1
		    if mod(l-m*kappa,2)==0
		    	value = value + x(1+(m-1)*M+(l-kappa*m)/2,1)*P(m+1,l-kappa*m+1,j)*C(m+1,i);
          end
        end
    end
...
function value = ulamevaleta(x,i,j)
	...
	for m=1:M
		for l=kappa*m:2*M+kappa*m-1
			if mod(l-m*kappa,2)==0
				value = value + m*x(1+(m-1)*M+(l-kappa*m)/2,1)*P(m+1,l-kappa*m+1,j)*S(m+1,i);
            end
        end
    end
...
function value = ulamevalphi(x,i,j,phival)
	...
	for m=1:M
		for l=kappa*m:2*M+kappa*m-1
			if mod(l-m*kappa,2)==0
				value = value + x(1+(m-1)*M+(l-kappa*m)/2,1)*Pp(m+1,l-kappa*m+1,j)*C(m+1,i);
            end
        end
    end
...
function value = ampcond(x)
	...
	for m=0:M-1
		for l=kappa*m:2*M+kappa*m-1
			if mod(l-m*kappa,2)==0
				value = value + x(1+2*M*M+m*M+(l-kappa*m)/2,1)*Pamp(m+1,l-kappa*m+1);
            end
        end
    end
