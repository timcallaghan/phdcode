...
double uphieval(double *x, double eta, double phi, double w, double a, double g, double Omega, double h0, int M, int kappa)
{
	double value = 0.0;
	int m,l,count;
	// The value of pi
	...
	for (m=1;m<=M;m++)
	{
		for (l=kappa*m;l<=2*M+kappa*m-1;l++)
		{
			if (((l-m*kappa)%2)!=0)
			{
				count = (m-1)*M+(l-kappa*m-1)/2;
				value += *(x+count)*plgndr(l,kappa*m,sin(phi))*sin(kappa*m*eta)*factor;
			}
		}
	}
...
void uphi(double w, double a, double g, double Omega, int kappa, double *eta, double *phi, double *B, double *y, int mm, int nn, double h0, int M)
{
	...
	for (ii=0; ii<mm; ii++) {
		for (jj=0; jj<nn; jj++) 
		{
			*(y+jj*mm+ii) = uphieval(B,*(eta+jj),*(phi+ii),w,a,g,Omega,h0,M,kappa);
		}
	}
                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  uphi(w,a,g,Omega,kappa,eta,phi,C,y,mrows,ncols,h0,M);
}
