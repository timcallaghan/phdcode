...
double ulam1(double *A, int Alen, int kappa, double phi)
{
	...
	for (i=kappa;i<kappa+Alen;i++)
	{
		answer += *(A+count)*plgndr(i,kappa,sin(phi));
		count++;
	}
...
double ulameval(double *x, double eta, double phi, double w, double a, double g, double Omega, double h0, int M, int kappa)
{
	double value = 0.0;
	int m,l,count;
	// The value of pi
...
void ulam(double w, double a, double g, double Omega, int kappa, double *eta, double *phi, double *A, double *y, int mm, int nn, double h0, int M)
{
	...
	for (ii=0; ii<mm; ii++) {
		for (jj=0; jj<nn; jj++) 
		{
			*(y+jj*mm+ii) = ulameval(A,*(eta+jj),*(phi+ii),w,a,g,Omega,h0,M,kappa);
		}
	}
                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  ulam(w,a,g,Omega,kappa,eta,phi,C,y,mrows,ncols,h0,M); 
}
