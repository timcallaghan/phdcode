...
function value = uphieval(x,i,j)
    ...
    for m=1:M
	    for l=m:2*M+m-1
		    if mod(l-m,2)~=0
		    	value = value + x(M*M+(m-1)*M+1+(l-m-1)/2,1)*P(m+1,l-m+1,j)*S(m+1,i);
          end
        end
    end     
...
function value = heval(x,i,j,phival)
    ...
    for m=0:M-1
	    for l=m:2*M+m-1
		    if mod(l-m,2)==0
		    	value = value + x(2*M*M+1+m*M+(l-m)/2,1)*P(m+1,l-m+1,j)*C(m+1,i);
          end
        end
    end
    % Finally, add in the base zonal flow structure
...
function value = uphievaleta(x,i,j)
    ...
    for m=1:M
	    for l=m:2*M+m-1
		    if mod(l-m,2)~=0
		    	value = value + x(M*M+(m-1)*M+1+(l-m-1)/2,1)*P(m+1,l-m+1,j)*C(m+1,i);
          end
        end
    end     
...
function value = hevaleta(x,i,j)
    ...
    for m=0:M-1
	    for l=m:2*M+m-1
		    if mod(l-m,2)==0
		    	value = value + x(2*M*M+1+m*M+(l-m)/2,1)*P(m+1,l-m+1,j)*S(m+1,i);
          end
        end
    end
    % Adjust for the negative sign from the derivative
...
function value = uphievalphi(x,i,j)
    ...
    for m=1:M
	    for l=m:2*M+m-1
		    if mod(l-m,2)~=0
		    	value = value + x(M*M+(m-1)*M+1+(l-m-1)/2,1)*Pp(m+1,l-m+1,j)*S(m+1,i);
          end
        end
    end     
...
function value = hevalphi(x,i,j,phival)
    ...
    for m=0:M-1
	    for l=m:2*M+m-1
		    if mod(l-m,2)==0
		    	value = value + x(2*M*M+1+m*M+(l-m)/2,1)*Pp(m+1,l-m+1,j)*C(m+1,i);
          end
        end
    end
