...
numunknowns = 3*M^2; % The number of unknowns in the nonlinear model
...
fixed=2*M*M+1
% Get the value of the fixed coeff
fixedval=coeffs(fixed,1);
% Resize the coeff vector
coeffsnew=zeros(numunknowns,1);
% Now fill out coeffsnew without translating the fixed coeff
    for z=1:numunknowns+1
        if (z<fixed)
            coeffsnew(z,1)=coeffs(z,1);
        elseif (z>fixed)
            coeffsnew(z-1,1)=coeffs(z,1);
        else
...
S=cacheS(M,kappa);
P=cacheL(mu,M,kappa);
%Pamp=cacheLamp(M,kappa); Not needed when we hold one fixed
