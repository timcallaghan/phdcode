...
void fsfield(int M, int N, int kappa, double *eta, double *phi, double *H, double *y, int mm, int nn)
{
	...
	for (ii=0; ii<mm; ii++) {
		for (jj=0; jj<nn; jj++) 
		{
			*(y+jj*mm+ii) = h1(H,*(phi+ii),*(eta+jj),M,N,kappa);
		}
	}
                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  fsfield(M,N,kappa,eta,phi,H,y,mrows,ncols); 
}
