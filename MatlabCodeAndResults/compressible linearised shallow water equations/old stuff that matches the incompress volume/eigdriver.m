...
N=10; % Truncation level (user defined)
...
pref=1.05*10^5; % Characteristic pressure value.
...
% Now, we want to do matching with the incompressible volumes/mass so we need to make sure
% we are using the correct characteristic parameter values. The main difference between the
% compressible and incompressible models is the different value of href. We adjust for this now
hrefincomp=8*10^3; % Characteristic length scale for incompressible model (m)
Frincomp=vref/sqrt(g*hrefincomp); % Froude number for incompressible model
Roincomp=vref/(2*Omega*aref); % Rossby number...not really essential to modify but no trouble to do so...
aincomp=aref/hrefincomp; % Nondimensional radius of sphere for incompressible model

% Now we can calculate the old Mass/Volume of the incompressible zonal flow for w and h0 specified. Note that
% we have nondimensionalised so that the dimensionless density=1 in the incompressible model, thus the volume
% is equal to the mass in this case.
Mb=f1(h0,w,Frincomp,Roincomp,aincomp);
...
% to the mass from the incompressible calculations. We use the in-built
% root finding procedure to do this.
h0 = fzero(@masserror,h0,[],w,a,gamma,Fr,Ma,Ro,Mb);
