...
const DP gamma = 1.4; // Ratio of specific heats
// Declare characteristic scales
const DP pref = 1.01325e05; // Characteristic pressure value. (1 atmosphere)
const DP rhoref = 1.29; // Characteristic density value.
...
const DP href = gamma*pref/((gamma-1.0)*g*rhoref);
...
const DP Ma = vref/sqrt(pref/rhoref); // Mach number
...
const int maxhalvings = 10; // Upper limit on number of halvings allowed
...
public: 
	...
	DP h0;
	// The zonal angular velocity (as a percentage of Williamson's w value
	DP w;
...
// The RH amplitude condition used in the linear solution
const DP Vzon = data_.Vzon;

// Now we can compute the base pseudo mass of the system
// using the formula
const DP Bval = (gamma-1.0)/gamma*pow(Ma/Fr,2.0);
const DP f1val = Bval*(arad+1.0);
const DP Mbp = (DP)kappa/( pi*( 2.0*pow(f1val*(gamma-1.0)/arad,2.0) + 2.0*Bval*f1val*(gamma-1.0)*gamma/arad + pow(Bval,2.0)*gamma*(2.0*gamma-1.0) ));
