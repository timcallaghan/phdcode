...
void dmass(const DP wavespeed, Vec_I_DP &vars,const DP phival,const int i,const int j,Mat_I_DP &Ceta,Mat_I_DP &Seta,Mat_I_DP &Cphi1,Mat_I_DP &Sphi1,Mat_I_DP &Cphi2, Mat_I_DP & Sphi2,Mat_IO_DP &jac)
{
    ...
    DP A1 = vars[5];
	DP A2 = (gamma-1.0)/gamma*vars[2];
	DP B1 = cos(phival)*vars[8]-(gamma-1.0)/gamma*vars[2]*sin(phival);
	DP B3 = (gamma-1.0)/gamma*vars[2]*cos(phival);
	DP C1 = (gamma-1.0)/gamma*(vars[3]+cos(phival)*vars[7]-sin(phival)*vars[1]);
