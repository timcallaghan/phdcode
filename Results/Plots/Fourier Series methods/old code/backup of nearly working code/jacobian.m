...
mval=0;
for nval=0:N
    df(numunknowns,2*M*N+nval+1)=-2.0*kappa*a^2/Vzon*dblquad(@intgrder,0,pi/2,0,pi/kappa,jacintol,@adaptlob,x(2*M*N+1:numunknowns,1),M,N,kappa,a,nval,mval);
end
for mval=1:M-1
    for nval=1:N
        df(numunknowns,2*M*N+mval*N+nval+1)=-2.0*kappa*a^2/Vzon*dblquad(@intgrder,0,pi/2,0,pi/kappa,jacintol,@adaptlob,x(2*M*N+1:numunknowns,1),M,N,kappa,a,nval,mval);
    end
end
