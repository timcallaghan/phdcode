...
function value = heval(x,i,j,phival)
    global M N kappa w h0 phi Ceta Seta Cphi1 Sphi1 Cphi2 Sphi2 numunknowns Sr Fr Ro fixed
    % Initialise to the zonal flow
    value = h0+w*(Fr^2)*(1/Ro+w)/2.0*(cos(phival)^2);
    % Add in eta-independent series components
    for n=2:N+1
    ...
    for m=1:M-1
        value = value - x(2*M*N+m*N+1,1)*Ceta(m,i)*(Cphi2(1,j)+1.0);
    end
    % Add in the rest of the series
    for m=1:M-1
	    for n=2:N
		    value = value + x(2*M*N+m*N+n,1)*Ceta(m,i)*((-1)^n)*(Cphi2(n,j)+Cphi2(n-1,j));
        end
    end
...
function value = hevaleta(x,i,j)
    ...
    % Add in the case when n=1
    for m=1:M-1
        value = value + kappa*m*x(2*M*N+m*N+1,1)*Seta(m,i)*(Cphi2(1,j)+1.0);
    end
    % Add in the rest of the series
    for m=1:M-1
	    for n=2:N
		    value = value - kappa*m*x(2*M*N+m*N+n,1)*Seta(m,i)*(-1)^(n)*(Cphi2(n,j)+Cphi2(n-1,j));
        end
    end
...
function value = hevalphi(x,i,j,phival)
    global M N kappa w h0 phi Ceta Seta Cphi1 Sphi1 Cphi2 Sphi2 numunknowns Sr Fr Ro fixed
    % Initialise to the zonal flow
    value = -w*(Fr^2)*(1/Ro+w)*cos(phival)*sin(phival);
    % Add in eta-independent series components
    for n=2:N+1
    ...
    % Add in the case when n=1
    for m=1:M-1
        value = value + x(2*M*N+m*N+1,1)*Ceta(m,i)*(2*Sphi2(1,j)+0.0);
    end
    % Add in the rest of the series
    for m=1:M-1
	    for n=2:N
		    value = value - x(2*M*N+m*N+n,1)*Ceta(m,i)*((-1)^n)*(2*n*Sphi2(n,j)+2*(n-1)*Sphi2(n-1,j));
        end
    end    
