...
mval=0;
for nval=0:N
    df(numunknowns,2*M*N+nval+1)=-2.0*kappa*a^2/Vzon*(dblquad(@intgrder1,0,pi/2,0,pi/kappa,jacintol,@adaptlob,x(2*M*N+1:numunknowns,1),M,N,kappa,a,nval,mval)+dblquad(@intgrder2,0,pi/2,0,pi/kappa,jacintol,@adaptlob,x(2*M*N+1:numunknowns,1),M,N,kappa,a,nval,mval));
end
for mval=1:M-1
    for nval=1:N
        df(numunknowns,2*M*N+mval*N+nval+1)=-2.0*kappa*a^2/Vzon*(dblquad(@intgrder1,0,pi/2,0,pi/kappa,jacintol,@adaptlob,x(2*M*N+1:numunknowns,1),M,N,kappa,a,nval,mval)+dblquad(@intgrder2,0,pi/2,0,pi/kappa,jacintol,@adaptlob,x(2*M*N+1:numunknowns,1),M,N,kappa,a,nval,mval));
    end
end
...
function value = ulameval(x,i,j,phival)
    ...
    for n=2:N+1
        value = value + x(n-1,1)*Cphi1(n,j);
    end
    % Add in the rest of the series
    for m=1:M-1
	    for n=1:N
		    value = value + x(m*N+n,1)*Ceta(m,i)*Cphi1(n,j);
        end
    end
...
function value = ulamevaleta(x,i,j)
    ...
    for m=1:M-1
	    for n=1:N
		    value = value - kappa*m*x(m*N+n,1)*Seta(m,i)*Cphi1(n,j);
        end
    end
...
function value = ulamevalphi(x,i,j,phival)
    ...
    % Add in eta-independent series components
    for n=2:N+1
        value = value - (2*n-1)*x(n-1,1)*Sphi1(n,j);
    end
    % Add in the rest of the series
    for m=1:M-1
	    for n=1:N
		    value = value - (2*n-1)*x(m*N+n,1)*Ceta(m,i)*Sphi1(n,j);
        end
    end
    ...
    for m=1:M-1
        for n=1:N
            if n==1
                ...
                yall(1,m*N+n)=A1*Ceta(m,i)*Cphi1(n,j)-A2*kappa*m*Seta(m,i)*Cphi1(n,j);
...
function y = dlammomen(wavespeed,vars,phival,i,j)
    ...
    % Terms related to m=0
    for n=2:N+1
        yall(1,n-1)=A1*Cphi1(n,j)-A3*(2*n-1)*Sphi1(n,j);
    end
    % Set up derivative with respect to the constant term H00
    yall(1,2*M*N+1)=0.0;
    % Now do the rest of the eta-independent terms
    ...
    for m=1:M-1
        for n=1:N
            if n==1
                % We need to use an alternative method for the H(i,1) derivatives
                yall(1,m*N+n)=A1*Ceta(m,i)*Cphi1(n,j)-A2*kappa*m*Seta(m,i)*Cphi1(n,j)-A3*(2*n-1)*Ceta(m,i)*Sphi1(n,j);
...
function y = dphimomen(wavespeed,vars,phival,i,j)
    ...
    for m=1:M-1
        for n=1:N
            if n==1
                % We need to use an alternative method for the H(i,1) derivatives
                yall(1,m*N+n)=A1*Ceta(m,i)*Cphi1(n,j);
