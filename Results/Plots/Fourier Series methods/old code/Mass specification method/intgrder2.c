...
void intgrder(int M, int N, double a, int kappa, double *phi, double eta, double *H, double *y, int mm, int nval, int mval)
{
	// Computational function that calculates the integrand
	...
	for (ii=0; ii<mm; ii++) 
	{
		...
		*(y+ii) = (pow(tempval/a,2.0)+2.0*tempval/a)*cos(phival)*dhdHval;
	}
}
                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  intgrder(M,N,a,kappa,phi,eta,H,y,mrows,nval,mval); 
}