...
% end of the graph.
k4mineigindex=1;
while cmat(1,k4mineigindex,2) < 0,
    k4mineigindex = k4mineigindex + 1;
end
% Now k4mineigindex is the index of the first encountered
% positive eigenvalue so store this val.
mineigenvalue=cmat(1,k4mineigindex,2);
% Now loop through the rest of the values and discard any that are negative
for i=k4mineigindex+1:3*N
    ...
    if cmat(1,i,2) < 0
        % do nothing
    % Now check if we have a new minimum value    
    elseif cmat(1,i,2) < mineigenvalue
        mineigenvalue=cmat(1,i,2);
        k4mineigindex=i;
    else
        % Do nothinf\g
    end
...
% end of the graph.
k5mineigindex=1;
while cmat(1,k5mineigindex,3) < 0,
    k5mineigindex = k5mineigindex + 1;
end
% Now k4mineigindex is the index of the first encountered
% positive eigenvalue so store this val.
mineigenvalue=cmat(1,k5mineigindex,3);
% Now loop through the rest of the values and discard any that are negative
for i=k5mineigindex+1:3*N
    % first check for values that are negative
    if cmat(1,i,3) < 0
        % do nothing
    % Now check if we have a new minimum value    
    elseif cmat(1,i,3) < mineigenvalue
        mineigenvalue=cmat(1,i,3);
        k5mineigindex=i;
    else
        % Do nothinf\g
    end
