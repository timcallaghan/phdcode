 ...
 * rhofield.c
 *
 * Computational function that calculates the density
 * field at a series of grid points in lat and long at the 
 * surface of a sphere
 *
...
void rhofield(double w, double Fr, double Ro, int kappa, double Ma, double gamma, double *eta, double *phi, double *H, int Hlen, double *y, int mm, int nn, double epsilon, double h0)
{
	...
	for (ii=0; ii<mm; ii++) {
		for (jj=0; jj<nn; jj++) 
		{
			h = hz(w,Fr,Ro, *(phi+ii),h0)+epsilon*cos( *(eta+jj)*kappa)*h1(H,Hlen, *(phi+ii));
			temp1 = (gamma-1.0)/gamma*pow(Ma/Fr,2.0)*h;
			*(y+jj*mm+ii) = pow(temp1,1.0/(gamma-1.0));
		}
	}
                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  double w,Fr,Ro,epsilon,h0,Ma,gamma;
  ...
  /* check to make sure the eleventh input argument is a scalar */
  if( !mxIsDouble(prhs[10]) || mxIsComplex(prhs[10]) ||
      mxGetN(prhs[10])*mxGetM(prhs[10])!=1 ) {
    mexErrMsgTxt("Input gamma must be a scalar.");
  }

  /*  get the scalar inputs kappa, w, Fr, Ro, epsilon, h0, Ma and gamma */
  ...
  rhofield(w,Fr,Ro,kappa,Ma,gamma,eta,phi,H,Hlen,y,mrows,ncols,epsilon,h0); 
}
